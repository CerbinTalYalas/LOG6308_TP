...
def train(data_train, k):
    train_model = csr_matrix((data_train['rating'], (data_train['user.id'], data_train['item.id'])), shape=(943, 1682),
                             dtype=float).toarray()

    mask = (train_model == 0.0)
    masked_arr = np.ma.masked_array(train_model, mask)
    item_means = np.mean(masked_arr, axis=0, dtype=float).filled(
        0.0)  # nan entries will replaced by the average rating for each item
